...
import DaysToSummer from '../../features/Summer/DaysToSummer';
...
const Hero = ({ variant = '', titleText, imageSrc, ...otherProps }) => (
  <div {...otherProps} className={styles.component + variant.split(' ').map(name => ' ' + (styles[name] || name)).join('')}>
    <div className={styles.daysToSummer} >
      <DaysToSummer description=' to summer!' days={{ one: 'day', many: 'days' }} />
    </div>
    <h2 className={styles.title}>{titleText}</h2>
    <img className={styles.image} src={imageSrc} />
    <div className={styles.happyHour}>
      <HappyHourAd promoDescription="Happy Hour, -20%!" title='Happy Hour' />
    </div>
  </div>
);
